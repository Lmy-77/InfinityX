...
local function getCurrentConfig() return string.format("%s%s%s", tostring(getgenv().DecompilerSettings.HideUpvalues or false), tostring(getgenv().DecompilerSettings.HideFunctionsNames or false), tostring(getgenv().DecompilerSettings.HideFunctionsLine or false)) end
...
local function decompile(s)
	...
	local response = request({
		...
		Body = HttpService:JSONEncode({
			...
			HideUpvalues = getgenv().DecompilerSettings.HideUpvalues or false,
			HideFunctionsNames = getgenv().DecompilerSettings.HideFunctionsNames or false,
			HideFunctionsLine = getgenv().DecompilerSettings.HideFunctionsLine or false,
		}),
		Method = "POST",
		Headers = {
